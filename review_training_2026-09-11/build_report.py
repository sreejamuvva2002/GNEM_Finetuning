...
runs=json.loads((P/'runs.json').read_text());hist=json.loads((P/'loss_history.json').read_text());masks=json.loads((P/'mask_reconstruction_summary.json').read_text())
order=['A_cpt','B_facts','C_answers','D_sql','BC_facts_answers','BD_facts_sql']
main={r['variant']:r for r in runs if '/run_v2_20260812_170423/'in r['path']}
L=['**Independent verification of GNEM fine-tuning — 11 September 2026**','',
...
'- Read every one of the 71 historical `.train.log` files under `v2-frozen-reference` (`b18313ae593995e8d415880603b3d355dd695ebd`). There are 54 logs with both a final training summary and completion metadata.','- Of these, 43 logs belong to v2 run directories: 34 completed and 9 incomplete. The completed v2 set comprises six original arms, three dose arms, and five seeds for each of A/B/C/D/BD. Earlier and smoke experiments are recorded separately in the CSV; they are not pooled into the v2 tables.','- Parsed raw loss dictionaries, gradients, learning rates, entropy, token accuracy, progress bars, and final metadata; cross-checked the nine original/dose runs against the separately archived adapter manifest. All nine timestamps and dataset hashes match.','- Inspected the trainer at all four commits named by completed v2 logs. The trainer source is byte-identical across those commits. Nine completed v2 runs report a dirty worktree, so a commit alone cannot prove the exact uncommitted runtime state; the 25 seeded runs report clean worktrees.','- Reconstructed assistant labels using the pinned Qwen tokenizer and the official source/template for the recorded TRL 1.9.2 version. This is a source-based reconstruction, not a replay of retained training batches.','- No adapter tensors, optimizer states, actual `training_args.bin`, or `trainer_state.json` are available in the current checkout/frozen file tree. Their historical hashes/configuration snapshots are available. I did not run a model forward/backward pass or recompute loss from checkpoint weights.','',
...
'|---|---:|---:|---:|---:|---:|---:|---:|']
for v in order:
 r=main[v];L.append(f"| {v} | {r['first_loss']:.6g} | {r['last_logged_loss']:.6g} | {r['train_loss']:.6g} | {r['progress_total']} | {r['epochs']:g} | {r['train_runtime']/60:.2f} min | {r['peak_vram_gb']:.2f} GB |")
L+=['','A has only two periodic loss observations, at epochs 1.696 and 3.348, because packing reduces the run to 24 optimizer steps and logging occurs every 10 steps. Its final summary is at epoch 4. Do not draw a detailed convergence trajectory from two observations. For B/C/D/BC/BD, the last logged windows are near, but not exactly at, the final epoch.','',
...
'|---|---:|---:|---:|---:|---:|']
for v in order:
 h=[r for r in hist if r['path']==main[v]['path']][-1]
 L.append(f"| {v} | {h['epoch']:g} | {h['mean_token_accuracy']:.2%} | {h['entropy']:.6g} | {h['grad_norm']:.6g} | {h['learning_rate']:.6g} |")
L+=['','**What the loss means**','',
...
for r in masks:L.append(f"| {r['variant']} | {r['input_rows']} | {r['retained_rows']} | {r['dropped_rows']} | {r['supervised_tokens_after']:,} | {r['expected_optimizer_steps_batch1_accum8_epochs3']} |")
...
for v in ['A_cpt','B_facts','C_answers','D_sql','BD_facts_sql']:
 rr={r['seed_label']:r for r in runs if r['v2_run']and r['completed']and r['variant']==v and r['seed_label']!='unsuffixed'};xs=[rr[str(s)]['train_loss']for s in [13,29,47,61,79]]
 L.append('| '+v+' | '+' | '.join(f'{x:.6g}'for x in xs)+f' | {statistics.mean(xs):.6g} ± {statistics.stdev(xs):.6g} |')
L+=['','![Seed-sweep losses](seed_loss_curves.png)','',
'**What cannot be concluded from the preserved evidence**','',
'- No validation-loss curves exist. We cannot calculate a train/dev loss gap, identify the minimum validation loss, or conclude that three epochs was optimal. Falling training loss is not a substitute.','- No intermediate checkpoints were saved by this driver. The final adapter was used; there is no demonstrated best-checkpoint selection by dev loss.','- Adapter hashes and header-derived metadata are preserved, but actual weights and optimizer states are absent here. A fresh held-out loss, parameter-delta/gradient inspection, optimizer-state audit, or exact replay would require those artifacts and a suitable model runtime.','- The driver defaults to deleting per-seed adapters after evaluation unless retention is explicitly requested. This reduces post-hoc verifiability.','- Eight incomplete v2 dose attempts show CUDA out-of-memory failures; another ends after four training steps without a completion record. These were excluded from completed-run statistics. They must not be reported as successful runs.','- An extremely low SQL training loss does not validate generated SQL on novel multi-operation questions or resolve the earlier identified gold/grading issues.','',
'**Recommended corrections before the next training run**','',
'1. Correct the dose interpretation in research notes; rerun only if that experiment is still needed.','2. Preserve the recovered B artifact with its proven hash and retain the corrupt archived file as evidence rather than silently replacing history.','3. Log raw and retained example counts, input tokens, supervised tokens, truncation counts, and actual construct exposure after all preprocessing. Assert the intended intervention reaches the loss labels.','4. Add a fixed dev set, periodic dev loss and execution-based business metrics, checkpoint retention, and an explicit dev-based selection rule. Keep final test evaluation separate.','5. Save actual training arguments, trainer state/log history, per-run package versions, effective chat template, label-mask samples, model revision, dataset hashes, seed, and adapter tensors.','6. Keep all per-seed adapters. Pin the base revision in the load call and record hardware/software settings needed to interpret numerical reproducibility.','7. Judge success using correct complete answers to realistic held-out questions alongside training diagnostics.','',
'**Evidence and reproduction files**','',
'- `extract_training_evidence.py`: reads historical Git objects and creates the complete run/loss tables and verbatim v2 logs.','- `check_token_lengths.py`: tokenizer-only length screening; its B result intentionally records the malformed archive.','- `reconstruct_masks.py`: uses the recovered B bytes and the official TRL training template to reconstruct masks and effective datasets.','- `mask_reconstruction_summary.json` and `mask_details_*.json`: per-arm and per-row label/truncation evidence.','- `log_manifest_crosscheck.json`: original-run log metadata versus archived adapter-manifest metadata.','- `historical_sources/`: trainer code at the recorded revisions, configuration snapshots, and adapter-manifest extract.','- `library_sources/`: downloaded official TRL 1.9.2 source and Qwen templates; local copies are evidence and are not imported into active v3 training.','- `raw_logs/`: verbatim copies of all 43 v2 training logs, including failures.','- `runs.csv`, `loss_history.csv`, corresponding JSON, and PNG/PDF plots: complete audit outputs.','',
