...
`certification_count` is loaded for **validation only**. `split` and `split_group` are leakage-control metadata, exposed solely through the separate `split_metadata()` accessor and never merged into a record.
...
```text
training generators scanned      = 0
forbidden full_kb call sites     = 0
```

Training generators do not exist yet (Phase 10+). **This run proves nothing about generators not yet written.** Every later training-data phase must re-run `static_check_no_training_generator_reaches_full_kb()` with its own module paths and report the scanned count.
...
| `model_facing_excludes_forbidden_fields` | PASS | 19 allowlisted fields; split/split_group/certification_count/lat/long absent by construction |
| `model_facing_is_allowlist_not_deletion` | PASS | projection built from MODEL_FACING_FIELDS |
| `certification_count_internal_only` | PASS | loaded for validation, never model-facing |
...
| `static_check_reports_coverage_honestly` | PASS | training generators scanned = 0; forbidden full_kb call sites = 0; proves nothing about generators that do not exist yet; this check must be re-run by every later training-data phase with its own paths |
