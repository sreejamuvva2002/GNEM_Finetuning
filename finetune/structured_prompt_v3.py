...
"""
import json
VERSION='structured_prompt_A002.1'
def output_instruction(item):
    ...
    if kind=='set':
        if len(item.get('target_columns',[]))>1:
            return 'Return only a JSON array of rows, with columns ordered as '+json.dumps(item['target_columns'])+'. Return [] if no record matches.'
        return 'Return only a JSON array of every requested value. Return [] if no record matches.'
