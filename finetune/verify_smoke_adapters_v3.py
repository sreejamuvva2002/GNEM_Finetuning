...
def main():
    ...
    for variant in FILES:
        directory=ROOT/'validation_v3/smoke_models'/f'{variant}.seed61'
        metric=json.loads((directory/'METRICS.json').read_text());assert metric['global_step']==2
        ...
        report[variant]={'adapter_sha256':hashlib.sha256(path.read_bytes()).hexdigest(),
            ...
            'reload_logits_identical':True,'optimizer_steps':2}
        print(variant,'PASS',difference,flush=True)
    (ROOT/'validation_v3/resumption/SMOKE_RELOAD_A002.json').write_text(json.dumps(report,indent=2)+'\n')
