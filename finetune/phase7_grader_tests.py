...
def _write_audit(db_sha, checks, battery_rows, sem_detail, bypasses,
    ...
    L = ["# GRADER_VALIDATION_v3\n",
         ...
         "The executor binds the four logical names to the selected scope's Phase 5 "
         "views as connection-local TEMP views, so the model never emits, and is "
         "never taught, a physical name like `train_kb_companies`.\n",
         ...
         "```\n",
         "`query_only` is set **after** the TEMP views, because creating them is "
